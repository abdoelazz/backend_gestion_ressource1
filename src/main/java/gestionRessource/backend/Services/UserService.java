...
import gestionRessource.backend.Utils.UserNotFoundException;
import gestionRessource.backend.Utils.WrongPasswordException;
...
public class UserService {
    ...
    public List<UserDTO> getEnseignants(String departement) {
        List<Enseignant> enseignants = enseignantRepository.findEnseignantByDepartementEquals(departement);
        List<Integer> ids = new ArrayList<>();
        for (int i =0; i<enseignants.size();i++){
            ids.add(enseignants.get(i).getId());
        }
    ...
    private List<UserDTO> getUserDTOS(List<User> users) {
        ...
        for(int i= 0;i<users.size();i++){
            UserDTO userDTO = UserDTO.builder()
                    ...
                    .build();
            Optional<Enseignant> enseignantOptional = enseignantRepository.findById(users.get(i).getId());
    ...
    public void deleteUser(Integer id) {
        ...
        if(user.getRole().name().equals("ENSEIGNANT") || user.getRole().name().equals("CHEF_DEPARTEMENT")){
            Enseignant enseignant = enseignantRepository.findById(user.getId()).get();
            List<Materiel> materiels = matereilRepository.findMaterielByEnseignant(enseignant);
            List<Message> messages = messageRepository.findAllMessageByEmetteurOrRecepteurOrFournisseur(user.getId());
            List<Affectation> affectations = affectationRepository.findAffectationByEnseignant(enseignant);
            //affectationRepository.deleteAll(affectations);
            messageRepository.deleteAll(messages);

            for(int i = 0;i<affectations.size();i++){
                affectations.get(i).setEnseignant(null);
