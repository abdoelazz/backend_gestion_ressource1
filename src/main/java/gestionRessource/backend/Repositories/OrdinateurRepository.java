...
import gestionRessource.backend.Models.Ordinateur;
...
public interface OrdinateurRepository extends JpaRepository<Ordinateur,Integer> {
    ...
    Ordinateur findOrdinateurByEnseignantId(int id);

    //@Query(nativeQuery = true , value =
//        "SELECT o.cpu,o.disque,o.ecran,o.ram FROM Ordinateur o INNER JOIN Enseignant e ON o.enseignant_id = e.id " +
...
//List<Ordinateur> findOrdinateurByDepartement(String departement);
    ...
    /*******************************************************************/
    @Query(nativeQuery = true,value = "" +
            "select im.*,m.code_barre,verifie,m.date_livraison,duree_garentie,marque,prix,appel_offre_id,enseignant_id,materiel_state, panne from materiel m,ordinateur im where im.id=m.id and m.verifie=1 and m.appel_offre_id IS null GROUP BY im.id")
    ...
    @Query(nativeQuery = true,value = "" +
            ...
            "              matp.prix,appel_offre_id,enseignant_id " +
    ...
    @Query(nativeQuery = true,value = "" +
            ...
            "              appel_offre_id,enseignant_id,panne, materiel_state,m.prix,m.marque" +
