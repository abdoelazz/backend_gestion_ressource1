...
//            "SELECT i.resolution,i.vitesse FROM Imprimente i INNER JOIN enseignant e ON i.enseignant_id = e.id " +
...
//    List<Imprimente> findImprimenteByDepartement(String departement);
    ...
    /****************************************************/
    @Query(nativeQuery = true,value = "" +
            "select im.*,m.code_barre,m.date_livraison,verifie,duree_garentie,marque,prix,appel_offre_id,enseignant_id, panne, materiel_state from materiel m,imprimente im  where im.id=m.id and m.verifie=1 and m.appel_offre_id IS null  GROUP BY im.id")
    List<Imprimente> getBesoinIM();
    @Query(nativeQuery = true,value = "" +
            "select im.*,m.code_barre,m.date_livraison,verifie,duree_garentie,matp.marque," +
            "   matp.prix,appel_offre_id,enseignant_id, panne, materiel_state" +
    ...
    @Query(nativeQuery = true,value = "" +
            ...
            "              appel_offre_id,enseignant_id,verifie,panne, materiel_state ,m.prix,m.marque" +
